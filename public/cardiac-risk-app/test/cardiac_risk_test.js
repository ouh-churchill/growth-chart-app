...
describe ('CardiacRisk', function() {
  ...
  describe ('processLabsData', function () {
    it ('invokes a function processLabResultsPageData when called.', function() {
      ...
      var deferredObj = 'objectDeferred';
      var smartObj = 'objectSmart';
      var labResultsObj = [['obj'],'nextPageObject'];
      var callBackFunction = function (deferred) {
      ...
      var args = {
        ...
        'smart' : smartObj,
        'nextPageObj' : labResultsObj[1],
        'callBackFunction' : callBackFunction
      };

      var mock = sinonSandbox.mock(CardiacRisk);
      mock.expects('processLabResultsPageData').once().withArgs(args, labResultsObj[0]);

      CardiacRisk.processLabsData(deferredObj, smartObj, labResultsObj, callBackFunction);
  ...
  describe ('processLabResultsPageData', function() {
    it ('invokes functions and validates labs are set',function (){

      var deferredObj = 'objectDeferred';
      var smartObj = {};
      smartObj.byCodes = function () {};
      var labResultsObj = [['obj'],'nextPageObject'];
      var callBackFunction = function (deferred) {
      ...
      var mock = sinonSandbox.mock(args.smart);
      mock.expects('byCodes').once().withExactArgs(labResultsObj[0], 'code').returns(loincCodes);
    ...
    it ('invokes functions and validates labs are set triggers mainCallback',function (){

      setPatientInfo('male',67,undefined,undefined,undefined,undefined,undefined,false,false,CardiacRisk.patientInfo);
      CardiacRisk.patientInfo.ldlDirect = undefined;
      CardiacRisk.patientInfo.ldlCalculated = undefined;
      var deferredObj = 'objectDeferred';
      var smartObj = {};
      smartObj.byCodes = function () {};
      var hasNext = function(){};
      var labResultsObj = [['obj'],{'hasNext': hasNext}];
      var callBackFunction = function (deferred) {
      ...
      var expectedCallBack = function (returnArguments) {
        expect(returnArguments.deferred).to.equal(deferredObj);
        expect(returnArguments.smart).to.equal(smartObj);
        expect(returnArguments.nextPageObj).to.equal(labResultsObj[1]);
        expect(returnArguments.callBackFunction).to.equal(callBackFunction);
      };
      ...
      mockCardiacRisk.expects('getCholesterolValue').once().withExactArgs(loincCodes('13457-7')).returns(162);
      mockCardiacRisk.expects('getSystolicBloodPressureValue').once().withExactArgs(loincCodes('8480-6')).returns(111);
      mockCardiacRisk.expects('isLabsNotAvailable').once().returns(false);
    ...
    it ('invokes functions and validates labs are set triggers getLabsNextPage',function (){
      ...
      var mockCardiacRisk = sinonSandbox.mock(CardiacRisk);
      mockCardiacRisk.expects('isLabsNotAvailable').once().returns(true);
      mockCardiacRisk.expects('getLabsNextPage').once().withArgs(args,expectedCallBack);
